      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 overflow-hidden rounded-lg border border-stone-800 bg-stone-900 shadow-xl animate-fade-in-up">
          ...
          <div className="p-1.5">
            ...
            {isStaff(user) && (
              <button onClick={() => { setOpen(false); navigate(ROUTES.adminDashboard); }} className="flex w-full cursor-pointer items-center gap-2.5 rounded-md px-3 py-2 text-sm text-stone-400 transition-colors hover:bg-stone-800 hover:text-stone-100">
                <Settings size={15} />
                Panou Admin
              </button>
            )}
